...
class SubsonicConverterFactory(
    ...
    override fun responseBodyConverter(
        ...
        Log.d(TAG, "registered converter for ${payloadType.typeName} (${payloadSerializer.descriptor.serialName})")
        ...
        return Converter<ResponseBody, SubsonicResponse<*>> { body ->
            ...
            val envelope = try {
                ...
                Log.w(TAG, "non-JSON response for ${payloadType.typeName}: '${raw.take(200)}'")
                ...
                Log.w(TAG, "malformed response for ${payloadType.typeName}: missing 'subsonic-response': '${raw.take(200)}'")
                throw SubsonicException(
                    code = SubsonicException.CODE_PARSE_ERROR,
            ...
            Log.d(
                TAG,
                "response ${payloadType.typeName}: status=$status version=$version type=$serverType serverVersion=$serverVersion openSubsonic=$openSubsonic",
            )

            if (status != "ok") {
                Log.w(TAG, "server error for ${payloadType.typeName}: code=${error?.code} msg='${error?.message}'")
                throw SubsonicException(
            ...
            Log.d(
                TAG,
                "decoded ${payloadType.typeName}: ${payloadJson.toString().take(1500)}",
            )
